...
- **component** (enum): One of [wp_plugin, wp_theme, wp_block, service_object, background_job, database, block_editor, interactivity_api, rest_api, custom_post_type, taxonomy, authentication, wp_cron, development_workflow, testing_framework, documentation, tooling]
...
- **wp_version** (string): WordPress version in X.Y.Z format
...
5. wp_version (if provided) must match X.Y.Z format
