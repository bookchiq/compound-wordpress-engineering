...
export function convertClaudeToQoder(
  ...
  const commands = plugin.commands
    ...
    .map((command) => convertCommand(command))
...
function convertCommand(command: ClaudeCommand): { name: string; content: string } {
  const frontmatter: Record<string, unknown> = {
  ...
  // Rewrite Claude paths to Qoder paths
  const body = rewriteClaudePaths(command.body.trim())

  const content = formatFrontmatter(frontmatter, body)
  return { name: normalizeName(command.name), content }
}
